#!/bin/sh
#   @file       apps.sh
#   @brief      Brief description
#   @author     Jozef Zuzelka <jozef.zuzelka@gmail.com>
#   @date
#    - Created: 10.04.2020 16:14
#    - Edited:  16.12.2021 18:52
#   @version    1.0.0
#   @par        SHELL: zsh 5.7.1 (x86_64-apple-darwin19.0)
#   @bug
#   @todo



echo "Unfortunately, you need to install these applications manually."


echo "Adobe Aftereffects"
echo "Adobe DNG Converter"
echo "Adobe Illustrator"
echo "Adobe Lightroom"
echo "Adobe Photoshop"
echo "Adobe Premiere"
echo "Autopano Giga"
echo "BetterTouchTool"
echo "Blackmagic Fusion"
echo "Charles"
echo "ChronoSync"
echo "DaisyDisk"
echo "EaseUS Data Recovery"
echo "EtreCheckPro"
echo "File Buddy"
echo "Google Earth"
echo "Hopper Disassembler"
echo "IDA Freeware"
echo "LRTimelapse 5"
echo "Latex"
echo "Lingon X"
echo "Macs Fan Control"
echo "Nik Collection"
echo "PTGui Pro"
echo "Parallels"
echo "Photomatix Pro 6"
echo "Sublime Text"
echo "Synalize It! Pro"
echo "Texpad"
#echo "tn3270 X.app"
echo "Topaz"
echo "Tuxera ntfs"
echo "uTorrent"
echo "Visual Studio"


