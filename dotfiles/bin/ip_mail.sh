...
if [ "$WAN_IP" != "$(cat ~/tmp/.lastip)" ] ; then
    ...
    # Posli mailom
    printf "HOSTNAME:\t$(hostname) \nUSER:\t$(whoami) \nDATE:\t$DATE \nWAN IP:\t$WAN_IP \nLAN IP:\t$LAN \nINTERFACE: $INTERFACE \n" | $MAIL_APP -s "Macbook's public IP" "$MYEMAIL" &

    # Posli polohu
    whereami | $MAIL_APP -s "Macbook's geolocation" "$MYEMAIL" &
    ...
    uuencode camphoto.zip camphoto.zip | mail -s "Macbook's photos from webcam" "$MYEMAIL" &
    sleep 5
    rm -r ~/tmp/camphoto
    rm camphoto.zip
fi
